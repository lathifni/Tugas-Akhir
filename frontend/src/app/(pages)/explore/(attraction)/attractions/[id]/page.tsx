...
import { fetchGeomTracking, fetchListGeomAttractions } from "@/app/(pages)/api/fetchers/attraction";
...
export default function AttractionIdPage({ params }: any) {
  ...
  const { data: dataAttractionDetail, isLoading: loadingListaGeomHomestay } = useQuery({
    queryKey: ['listGeomAttraction'],
    queryFn: fetchListGeomAttractions,
    select: (data) =>
    data?.filter((item: { id: string, name: string, type: string, price: number, description: string, lat: number, lng: number, geom: string }) => item.id === params.id),
  })
        ...
        <div className="w-full h-full px-1 py-1 mb-3 lg:p-0 lg:mb-0 lg:mr-3 sm:py-3 lg:w-2/3 bg-white rounded-lg">
          ...
          { dataAttractionDetail && (
            <MapAttraction isManualLocation={isManualLocationClicked} setIsManualLocation={setIsManualLocationClicked}
              ...
              userLocation={userLocation} detailEventId={params.id} selectedAttractionId={dataAttractionDetail[0].id} />
            )}
          </div>
        </div>
        {dataAttractionDetail && (
          <div className="py-5 flex flex-col lg:w-1/3 items-center bg-white rounded-lg">
            <div className="text-2xl text-center justify-center font-bold">
              <h1 className="">{dataAttractionDetail[0].name}</h1>
            </div>
            <div className="w-full px-5">
              <div>
                <p dangerouslySetInnerHTML={{ __html: dataAttractionDetail[0].description.replace(/(?:\r\n|\r|\n)/g, '<br>') }} />
                <br />
                <p>Price: {dataAttractionDetail[0].price}</p>
