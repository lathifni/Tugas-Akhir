...
export default function DeleteDialogReservation({ isOpen, setIsOpen, rowDelete, setNotification, onSuccessfulDelete }: Props) {  
  const handleDelete = () => {
    useAxiosAuth.delete(`/reservation/${rowDelete.id}`)
    .then((res) => {
      setIsOpen(!isOpen)
  ...
  return (
    <Dialog open={isOpen}>
      <div className="px-10 py-2 justify-center text-center">
        <h1 className="text-3xl text-center font-bold">Confirmation Delete Reservation</h1>
        <p>
          Are you sure to delete reservation <strong>{rowDelete.name}</strong> id <strong>{rowDelete.id}</strong> ?
        </p>
      </div>
