...
export default function TableReservationExplore({ columns, data, onDelete, }: Props) {
          ...
          {table.getRowModel().rows.map(row => (
            <tr key={row.id} className='px-6 py-3 hover:bg-gray-200'>
              ...
              <td className='px-1 py-2'>
                <Link href={`/explore/reservation/${row.original.id}`}>
                ...
                {row.original.status === "Waiting Confirmation Date" && (
                  <button 
                    onClick={() => onDelete(row.original)}
                    className='mx-2 border-2 border-red-500 p-2 rounded-lg text-red-500 hover:text-white hover:bg-red-500 '>
                    <FontAwesomeIcon icon={faTrash} style={{ fontSize: '1.4em' }} />
                  </button>
                )}
              </td>
            </tr>
