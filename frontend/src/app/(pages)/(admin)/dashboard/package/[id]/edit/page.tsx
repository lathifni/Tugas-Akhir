...
import { fetchAllPackageInformationById, fetchAllPackageType, fetchListAllService } from "@/app/(pages)/api/fetchers/package"
...
const packageSchema = z.object({
  ...
  price: z.union([
    z.string().min(1, 'Price cannot be empty for strings'),
    z.number().min(1, 'Price cannot be less than 1'),
  ]),
  min_capacity: z.union([
    z.string().min(1, 'Minimum capacity cannot be empty for strings'),
    z.number().min(1, 'Minimum capacity cannot be less than 1'),
  ]),
...
export default function PackageIdEditPage({ params }: any) {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['AllPackageInformationById'],
    queryFn: () => fetchAllPackageInformationById(params.id)
  })
  ...
  useEffect(() => {
    if (data) {
      console.log(data);
      setFormDataInput({
        package_name: data.package[0].name,
        package_type: data.package[0].type_id,
        min_capacity: data.package[0].min_capacity,
        contact_person: data.package[0].contact_person,
        description: data.package[0].description,
        price: data.package[0].price,
      })
      setPackageActivities(data.package_activities)
      setPackageServices(data.package_service)
      setPackageDay(data.package_day)
      setMaxDay(data.package[0].max_day)
    }
  }, [data])
  ...
  const handleSubmit = async () => {
    ...
    let responseCover, responseGallery, responseVideo
    if (cover.length > 0) {
      const formDataCover = new FormData()
      formDataCover.append('category', category);
      formDataCover.append('images[0]', cover[0]!.file)
      responseCover = await axios.post("/api/images", formDataCover);
    }
    if (gallery.length > 0) {
    ...
    if (video.length > 0) {
      ...
      responseVideo = await axios.post("/api/videos", formDataVideo);
    }
    ...
    const response = await useAxiosAuth.put('package', dataForm)
    console.log(response.data);
    
    if (response.status == 200) router.push('/dashboard/package')
  }
  ...
  const handleRemoveActivities = (activityId: string, dayIndex: number) => {    
    const activitiesForDay = packageActivities.filter(activity => activity.day == dayIndex);
    if (activitiesForDay.length == 1) {
      const updatedPackageDay = packageDay
      .filter(day => day.day !== dayIndex.toString())
      .map((day) => {
        // Mengurangi nilai day setelah day yang dihapus agar urutannya tetap berkesinambungan
        const dayNumber = Number(day.day);
        if (dayNumber > dayIndex) {
          return { ...day, day: (dayNumber - 1).toString() }; // Turunkan nilai day setelah penghapusan
        }
        return day;
      });
      
    setPackageDay(updatedPackageDay);
    // Kurangi maxDay
    setMaxDay(updatedPackageDay.length);
    ...
    updatedActivities = updatedActivities
      .map(activity => {
        if (activity.day > dayIndex) {
          console.log(activity.day);
          console.log(activity.day-1);
          return { ...activity, day: activity.day - 1 }; // Turunkan nilai day untuk aktivitas yang lebih besar dari dayIndex
        }
        return activity;
      });
  ...
  if (isLoading) {
    // Tampilkan loading state saat data belum didapat
    return <div>Loading...</div>;
  }
  
  if (data && dataListAllObject) {
    return (
      <>
        <div className="flex flex-col lg:flex-row m-1 sm:m-3 lg:m-5">
          <div className="w-full h-full px-2 py-3 mb-4 lg:p-4 lg:mb-0 lg:mr-3 lg:w-5/12 bg-white rounded-lg">
            <h1 className="text-2xl font-bold text-center">Edit Package</h1>
            <div className="mb-4">
              <label>Package Name</label>
              <input type="text" name='package_name' onChange={handleChange} value={formDataInput.package_name}
            ...
            <div className="mb-4">
              <label>Price</label>
              <input type="number" name='price' onChange={handleChange} min="0" value={formDataInput.price}
              className="bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full"/>
            </div>
            <div className="mb-4">
              <label>Minimal Capacity</label>
              <input type="number" name='min_capacity' onChange={handleChange} min="0" value={formDataInput.min_capacity}
              className="bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full"/>
            </div>
            <div className="mb-4">
            ...
            <div className="mb-4">
              ...
              <textarea id="message" name='description' onChange={handleChange} value={formDataInput.description}
            ...
            <div className="mb-4">
              <label>Package Cover Saved</label>
              <div className="bg-gray-100 border-2 border-blue-500 border-dashed rounded-lg">
                <img className="p-8" src={`/photos/package/${data.package[0].cover_url}`} alt={data.package[0].cover_url} />
              </div>
            </div>
            <div className="mb-4">
              <label>Change Package Cover</label>
            ...
            <div className="mb-4">
              ...
              <FileEdit galleries={data.package_gallery} folder="package" onDeleteImage={handleDeleteGallery} fileType={"image"} />
            </div>
            <div className="mb-4">
              <label>Gallery</label>
            ...
            <div className="mb-4">
              <label>Video Saved</label>
              <div className="bg-gray-100 border-2 border-blue-500 border-dashed rounded-lg">
                {/* <img className="p-8" src={`/photos/package/${data.package[0].cover_url}`} alt={data.package[0].cover_url} /> */}
                <video className="p-8" controls>
                    <source src={`/videos/package/${data.package[0].video_url}`} />
                  </video>
              </div>
              {/* <FileEdit galleries={[{ url: data.package[0].video_url }]} folder="package" onDeleteImage={handleDeleteImage} fileType={"video"} /> */}
            </div>
            <div className="mb-4">
                                ...
                                <tr key={index} className="mb-1">
                                  ...
                                  <td className="flex justify-center items-center">
                                    <button className="border-solid border-2 border-red-500 rounded-lg text-red-500 px-2 py-1 hover:text-white hover:bg-red-500"
                                      onClick={() => handleRemoveActivities(activity.activity, dayIndex + 1)}>
