...
export default function DeleteDialogAnnouncement({ isOpen, setIsOpen, rowDelete, setNotification, onSuccessfulDelete }: Props) {
  const handleDelete = () => {
    useAxiosAuth.delete(`/gtp/announcement/${rowDelete.id}`)
    .then((res) => {
      if (res.status == 200) {
        setIsOpen(!isOpen)
